...
// One actor per accepted Dreo device. Dreo ceiling fans only speak to the cloud, which the
// standalone `dreo2mqtt` sidecar bridges onto the local MQTT broker. This actor consumes that:
// device state arrives on "<base>/<serial>"; commands are published to "<base>/<serial>/set"
// for the sidecar to forward to the device.
public sealed class DreoDeviceActor : ReceiveActor, IWithTimers
{
    ...
    private readonly DreoDeviceCredential _cred;
    ...
    private DreoTransport? _transport;
    ...
    public static Props Props(DreoDeviceCredential cred, Guid deviceId, string brokerHost, int brokerPort, string baseTopic) =>
        Akka.Actor.Props.Create(() => new DreoDeviceActor(cred, deviceId, brokerHost, brokerPort, baseTopic));

    public DreoDeviceActor(DreoDeviceCredential cred, Guid deviceId, string brokerHost, int brokerPort, string baseTopic)
    {
        _cred = cred;
        _deviceId = deviceId;
        ...
        {
            if (_transport != DreoTransport.Local)
            {
    ...
    private async Task ConnectAsync()
    {
        try
        {
            ...
            {
                _log.Warning("Dreo broker connect for {Serial}: {Code}", _cred.Serial, result.ResultCode);
                SetTransport(DreoTransport.Offline);
                Timers.StartSingleTimer("reconnect", ConnectToBroker.Instance, ReconnectDelay);
                return;
            }
            ...
            StartInboundStream();
            StartOutboundStream();
            SetTransport(DreoTransport.Local); // "online"
            _log.Info("Dreo {Serial} consuming {Topic} from broker {Host}:{Port}",
                _cred.Serial, _stateTopic, _brokerHost, _brokerPort);
        }
        catch (Exception ex)
        {
            _log.Error(ex, "Failed to connect Dreo {Serial} to broker", _cred.Serial);
            SetTransport(DreoTransport.Offline);
            Timers.StartSingleTimer("reconnect", ConnectToBroker.Instance, ReconnectDelay);
        }
    }
    ...
    private void StartInboundStream()
    {
        ...
        ChannelSource.FromReader<string>(_inboundChannel!.Reader)
            .Select(payload =>
            {
                try { return DreoStateMapper.MapState(payload); }
                catch (Exception ex)
                {
                    log.Warning(ex, "Failed to map Dreo state payload for {Serial}", serial);
    ...
    private void StartOutboundStream()
    {
        ...
        ChannelSource.FromReader<DeviceCommand>(_outboundChannel!.Reader)
            .Select(cmd => DreoCommandBuilder.Build(cmd.CapabilityKey, cmd.Value))
            .Where(payload => payload != null)
            .SelectAsync(1, async payload =>
            {
                await mqttClient.PublishStringAsync(commandTopic, payload!,
                    cancellationToken: CancellationToken.None);
                log.Info("Sent Dreo command to {Topic}: {Payload}", commandTopic, payload);
                return payload;
            })
